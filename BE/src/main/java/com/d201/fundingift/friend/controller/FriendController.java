...
public class FriendController {
    ...
    @Operation(summary = "친구 목록 동기화",
            description = "카카오톡 친구 목록을 통해 친구 목록을 동기화 합니다. `Token`"
    )
    @ApiResponses(value = {
    ...
    친한친구, 이름 순으로 정렬하여 반환합니다.
    """
    )
    @GetMapping
    public SuccessResponse<GetFriendsResponse> getFriends() {
        GetFriendsResponse friendsResponse = friendService.getFriends(securityUtil.getConsumerId());
    ...
    public SuccessResponse<?> toggleFavorite(@PathVariable("to-consumer-id") Long toConsumerId) {
        friendService.toggleFavorite(securityUtil.getConsumerId(), toConsumerId);
