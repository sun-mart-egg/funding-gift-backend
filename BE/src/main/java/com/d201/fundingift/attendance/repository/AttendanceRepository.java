...
public interface AttendanceRepository extends JpaRepository<Attendance, Long> {
    ...
    @Query("select a from Attendance a " +
            "where a.fundingEntity.id = :fundingId and a.deletedAt is null")
    Slice<Attendance> findAllByFundingIdAndDeletedAtIsNull(@Param("fundingId") Long fundingId, Pageable pageable);

    @Query("select f from Attendance a right join  a.fundingEntity f " +
            "where a.consumer.id = :consumerId and a.deletedAt is null")
    Slice<FundingEntity> findAllByConsumerIdAndAndDeletedAtIsNull(@Param("consumerId") Long consumerId, Pageable pageable);
